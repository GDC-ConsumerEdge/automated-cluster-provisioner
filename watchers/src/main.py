...
def zone_watcher(req: flask.Request):
    ...
    if edgecontainer_api_endpoint_override:
        op = client_options.ClientOptions(
            api_endpoint=urlparse(edgecontainer_api_endpoint_override).netloc
        )
...
def cluster_watcher(req: flask.Request):
    ...
    if edgenetwork_api_endpoint_override:
        op = client_options.ClientOptions(
            api_endpoint=urlparse(edgenetwork_api_endpoint_override).netloc
        )
    ...
    if gkehub_api_endpoint_override:
        op = client_options.ClientOptions(
            api_endpoint=urlparse(gkehub_api_endpoint_override).netloc
        )
    ...
    for proj_loc_key in config_zone_info:
        ...
        for store_id in config_zone_info[proj_loc_key]:
            ...
            rw = zone_cluster_list[
                0
            ].maintenance_policy.window.recurring_window  # cluster in this GDCE zone
            ...
            if (
                ...
                defined_exclusion_windows = (
                    MaintenanceExclusionWindow.get_exclusion_windows_from_sot(
                        store_info
                    )
                )
                ...
                actual_exclusion_windows = (
                    MaintenanceExclusionWindow.get_exclusion_windows_from_api_response(
                        mw
                    )
                )
...
def zone_active_metric(req: flask.Request):
    ...
    token = get_git_token_from_secrets_manager(
        params.secrets_project_id, params.git_secret_id
    )
    intent_reader = ClusterIntentReader(
        params.source_of_truth_repo,
        params.source_of_truth_branch,
        params.source_of_truth_path,
        token,
    )
    zone_config_fio = intent_reader.retrieve_source_of_truth()
    rdr = csv.DictReader(
        io.StringIO(zone_config_fio)
    ...
    for row in rdr:
        ...
        if (
            b_zone_found
            and zone.globally_unique_id is not None
            and len(zone.globally_unique_id.strip()) > 0
        ):
...
def verify_zone_state(store_id: str, recreate_on_delete: bool) -> bool:
    ...
    if (
        state == Zone.State.READY_FOR_CUSTOMER_FACTORY_TURNUP_CHECKS
        or state == Zone.State.CUSTOMER_FACTORY_TURNUP_CHECKS_STARTED
    ):
